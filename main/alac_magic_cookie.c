...
void build_alac_magic_cookie(uint8_t* cookie, const audio_format_t* fmt) {
  ...
  uint32_t frame_length = fmt->max_samples_per_frame ? fmt->max_samples_per_frame : (fmt->frame_size > 0 ? (uint32_t)fmt->frame_size : 352);
  uint8_t  compatible_version = 0;
  uint8_t  bit_depth = fmt->sample_size ? fmt->sample_size : (fmt->bits_per_sample ? fmt->bits_per_sample : 16);
  ...
  uint8_t  num_channels = fmt->num_channels ? fmt->num_channels : (fmt->channels ? fmt->channels : 2);
  ...
  uint32_t sample_rate = fmt->sample_rate_config ? fmt->sample_rate_config : (fmt->sample_rate ? fmt->sample_rate : 44100);
