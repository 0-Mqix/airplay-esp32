...
void audio_output_flush(void) {
  ...
  if (flush_complete_sem) {
    xSemaphoreTake(flush_complete_sem, pdMS_TO_TICKS(100));
  }
}
...
static void playback_task(void *arg) {
  ...
  int16_t *silence = heap_caps_calloc(FRAME_SAMPLES * 2, sizeof(int16_t),
  ...
  while (true) {
    ...
    if (xTaskNotifyWait(0, FLUSH_NOTIFY_BIT, &notify_value, pdMS_TO_TICKS(1)) ==
        pdTRUE) {
      if (notify_value & FLUSH_NOTIFY_BIT) {
        ...
        flush_pending = false;
        if (flush_complete_sem) {
          xSemaphoreGive(flush_complete_sem);
        }
        continue;
      }
    }
...
void audio_output_start(void) {
  ...
  if (!flush_complete_sem) {
    flush_complete_sem = xSemaphoreCreateBinary();
  }

  xTaskCreatePinnedToCore(playback_task, "audio_play", TASK_PLAYBACK_STACK,
                          NULL, TASK_PLAYBACK_PRIORITY, &playback_task_handle,
                          TASK_PLAYBACK_CORE);
}
