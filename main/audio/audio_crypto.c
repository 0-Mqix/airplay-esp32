...
int audio_crypto_decrypt_rtp(
  ...
  if (!encrypt || !input || !output) { return -1; }

  if (encrypt->type == AUDIO_ENCRYPT_NONE) {
    if (input_len > output_capacity) { return -1; }
  ...
  if (encrypt->type == AUDIO_ENCRYPT_AES_CBC) {
    ...
    if (encrypted_len > 0) {
      ...
      ret = mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_DECRYPT, encrypted_len, iv, input, output);
      mbedtls_aes_free(&aes);
      if (ret != 0) { return -1; }
    }

    if (remainder > 0) { memcpy(output + encrypted_len, input + encrypted_len, remainder); }
  ...
  if (encrypt->type == AUDIO_ENCRYPT_CHACHA20_POLY1305) {
    ...
    if (!full_packet || full_packet_len < 12) { return -1; }

    if (input_len < crypto_aead_chacha20poly1305_ietf_ABYTES + 8) { return -1; }
    ...
    int ret = crypto_aead_chacha20poly1305_ietf_decrypt(output, &decrypted_len, NULL, input, ciphertext_len, aad, aad_len, nonce, encrypt->key);
    if (ret != 0) { return -1; }
...
int audio_crypto_decrypt_buffered(const audio_encrypt_t* encrypt, const uint8_t* packet, size_t packet_len, uint8_t* output, size_t output_capacity) {
  if (!packet || !output) { return -1; }
  ...
  if (ciphertext_len > output_capacity + crypto_aead_chacha20poly1305_ietf_ABYTES) { return -1; }

  unsigned long long decrypted_len = 0;
  int ret = crypto_aead_chacha20poly1305_ietf_decrypt(output, &decrypted_len, NULL, ciphertext, ciphertext_len, aad, aad_len, nonce, encrypt->key);
  if (ret != 0) { return -1; }
