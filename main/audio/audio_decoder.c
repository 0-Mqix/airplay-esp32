...
typedef enum { AUDIO_DECODER_NONE = 0, AUDIO_DECODER_PCM, AUDIO_DECODER_ALAC, AUDIO_DECODER_AAC } audio_decoder_kind_t;
...
static bool codec_is_aac(const char* codec) {
  if (!codec) { return false; }
  return strstr(codec, "AAC") != NULL || strstr(codec, "aac") != NULL || strstr(codec, "mpeg4-generic") != NULL;
}

static bool aac_has_adts_header(const uint8_t* data, size_t len) { return len >= 2 && data[0] == 0xFF && (data[1] & 0xF0) == 0xF0; }
...
audio_decoder_t* audio_decoder_create(const audio_decoder_config_t* config) {
  ...
  if (codec_is_alac(config->format.codec)) {
    ...
    esp_alac_dec_cfg_t alac_cfg = {.codec_spec_info = decoder->alac_magic_cookie, .spec_info_len = ALAC_MAGIC_COOKIE_SIZE};

    esp_audio_err_t err = esp_alac_dec_open(&alac_cfg, sizeof(alac_cfg), &decoder->alac_decoder);
    ...
    aac_cfg.bits_per_sample = config->format.bits_per_sample ? config->format.bits_per_sample : 16;
    aac_cfg.no_adts_header = false;
    aac_cfg.aac_plus_enable = false;

    esp_audio_err_t err = esp_aac_dec_open(&aac_cfg, sizeof(aac_cfg), &decoder->aac_decoder);
    if (err != ESP_AUDIO_ERR_OK) {
      ESP_LOGE(TAG, "Failed to open AAC decoder: %d", err);
      decoder->aac_decoder = NULL;
      decoder->kind = AUDIO_DECODER_NONE;
    }
  } else if (strcmp(config->format.codec, "L16") == 0 || strcmp(config->format.codec, "PCM") == 0) {
...
int audio_decoder_decode(
  ...
  if (!decoder || !input || !output || output_capacity_samples == 0) { return -1; }

  int channels = decoder->format.channels;
  if (channels <= 0) { channels = MAX_FALLBACK_CHANNELS; }

  if (decoder->kind == AUDIO_DECODER_PCM) {
    size_t decoded_samples = input_len / (channels * sizeof(int16_t));
    if (decoded_samples > output_capacity_samples) { decoded_samples = output_capacity_samples; }

    const int16_t* src = (const int16_t*)input;
    for (size_t i = 0; i < decoded_samples * channels; i++) { output[i] = ntohs(src[i]); }

    if (info) { info->channels = channels; }
    return (int)decoded_samples;
  }

  if (decoder->kind == AUDIO_DECODER_ALAC) {
    if (!decoder->alac_decoder) { return -1; }

    esp_audio_dec_in_raw_t raw = {.buffer = (uint8_t*)input, .len = (uint32_t)input_len, .consumed = 0, .frame_recover = ESP_AUDIO_DEC_RECOVERY_NONE};
    ...
    esp_audio_err_t err = esp_alac_dec_decode(decoder->alac_decoder, &raw, &frame, &dec_info);
    if (err != ESP_AUDIO_ERR_OK) { return -1; }

    int dec_channels = dec_info.channel > 0 ? dec_info.channel : channels;
    if (dec_channels <= 0) { dec_channels = MAX_FALLBACK_CHANNELS; }

    size_t decoded_samples = frame.decoded_size / (dec_channels * sizeof(int16_t));
    if (decoded_samples > output_capacity_samples) { decoded_samples = output_capacity_samples; }
  ...
  if (decoder->kind == AUDIO_DECODER_AAC) {
    ...
    if (!aac_has_adts_header(input, input_len)) {
      ...
      if (!decoder->aac_frame_buffer || decoder->aac_frame_buffer_size < needed) {
      ...
      build_adts_header(decoder->aac_frame_buffer, input_len, decoder->format.sample_rate, decoder->format.channels);
    ...
    esp_audio_err_t err = esp_aac_dec_decode(decoder->aac_decoder, &raw, &frame, &dec_info);
    if (err != ESP_AUDIO_ERR_OK) { return -1; }
...
bool audio_decoder_is_aac(const audio_decoder_t* decoder) { return decoder && decoder->kind == AUDIO_DECODER_AAC; }

bool audio_decoder_is_alac(const audio_decoder_t* decoder) { return decoder && decoder->kind == AUDIO_DECODER_ALAC; }
