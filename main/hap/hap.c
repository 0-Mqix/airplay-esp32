...
esp_err_t hap_init(void) {
  ...
  if (err == ESP_OK) { err = nvs_get_blob(nvs, NVS_KEY_SECRET, g_device_secret_key, &sec_len); }
  ...
  if (err != ESP_OK || pub_len != HAP_ED25519_PUBLIC_KEY_SIZE || sec_len != HAP_ED25519_SECRET_KEY_SIZE) {
    ...
    if (err == ESP_OK) { err = nvs_set_blob(nvs, NVS_KEY_SECRET, g_device_secret_key, HAP_ED25519_SECRET_KEY_SIZE); }
...
const uint8_t* hap_get_public_key(void) { return g_device_public_key; }
