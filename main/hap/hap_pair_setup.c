...
hap_pair_setup_m1(hap_session_t* session, const uint8_t* input, size_t input_len, uint8_t* output, size_t output_capacity, size_t* output_len) {
  ...
  if (flags && flags_len == 1) { transient = (flags[0] & 0x10) != 0; }
  session->pair_setup_transient = transient;

  if (session->srp) { srp_session_free(session->srp); }
...
hap_pair_setup_m3(hap_session_t* session, const uint8_t* input, size_t input_len, uint8_t* output, size_t output_capacity, size_t* output_len) {
  ...
  if (!tlv8_decode_concat(input, input_len, TLV_TYPE_PUBLIC_KEY, client_pk, sizeof(client_pk), &pk_len)) {
    ESP_LOGE(TAG, "Missing client public key in M3");
    return ESP_ERR_INVALID_ARG;
  }

  uint8_t client_proof[64];
  if (!tlv8_decode_concat(input, input_len, TLV_TYPE_PROOF, client_proof, sizeof(client_proof), &proof_len)) {
    ESP_LOGE(TAG, "Missing client proof in M3");
    return ESP_ERR_INVALID_ARG;
  }

  esp_err_t err = srp_verify_client(session->srp, client_pk, pk_len, client_proof, proof_len);
  if (err != ESP_OK) {
  ...
  if (session->pair_setup_transient) {
    ...
    hap_hkdf_sha512((uint8_t*)"Control-Salt", 12, srp_key, srp_key_len, (uint8_t*)"Control-Read-Encryption-Key", 27, session->encrypt_key, 32);
    hap_hkdf_sha512((uint8_t*)"Control-Salt", 12, srp_key, srp_key_len, (uint8_t*)"Control-Write-Encryption-Key", 28, session->decrypt_key, 32);
...
hap_pair_setup_m5(hap_session_t* session, const uint8_t* input, size_t input_len, uint8_t* output, size_t output_capacity, size_t* output_len) {
  ...
  if (!tlv8_decode_concat(input, input_len, TLV_TYPE_ENCRYPTED_DATA, encrypted, sizeof(encrypted), &encrypted_len)) {
  ...
  hap_hkdf_sha512((uint8_t*)"Pair-Setup-Encrypt-Salt", 23, srp_key, srp_key_len, (uint8_t*)"Pair-Setup-Encrypt-Info", 23, setup_key, 32);

  uint8_t            nonce[12] = {0, 0, 0, 0, 'P', 'S', '-', 'M', 's', 'g', '0', '5'};
  uint8_t            decrypted[512];
  unsigned long long decrypted_len = 0;

  if (crypto_aead_chacha20poly1305_ietf_decrypt(decrypted, &decrypted_len, NULL, encrypted, encrypted_len, NULL, 0, nonce, setup_key) != 0) {
