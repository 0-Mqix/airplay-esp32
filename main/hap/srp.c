...
static void trim_leading_zeros(const uint8_t* in, size_t in_len, const uint8_t** out, size_t* out_len) {
  while (in_len > 1 && *in == 0) {
    in++;
    in_len--;
  }
...
esp_err_t srp_start(srp_session_t* session, const char* username, const char* password) {
  if (!session || !username || !password) { return ESP_ERR_INVALID_ARG; }
...
const uint8_t* srp_get_salt(srp_session_t* session) { return session ? session->salt : NULL; }
...
srp_verify_client(srp_session_t* session, const uint8_t* client_public_key, size_t client_pk_len, const uint8_t* client_proof, size_t proof_len) {
  if (!session || !client_public_key || !client_proof || proof_len < SRP_PROOF_BYTES) { return ESP_ERR_INVALID_ARG; }
  ...
  memcpy(session->client_public_key + (SRP_PRIME_BYTES - client_pk_len), client_public_key, client_pk_len);
  ...
  uint8_t expected_m1[64];
  {
    ...
    // H(N) ^ H(g)
    uint8_t h_Ng_xor[64];
    for (int i = 0; i < 64; i++) { h_Ng_xor[i] = h_N[i] ^ h_g[i]; }
    ...
    compute_m1(expected_m1, h_Ng_xor, h_I, salt_ptr, salt_len, A_bytes, A_len, B_bytes, B_len, session->session_key, 64);
  }
