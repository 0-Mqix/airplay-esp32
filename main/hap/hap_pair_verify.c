...
hap_pair_verify_m1(hap_session_t* session, const uint8_t* input, size_t input_len, uint8_t* output, size_t output_capacity, size_t* output_len) {
  ...
  hap_hkdf_sha512(
    (uint8_t*)"Pair-Verify-Encrypt-Salt",
    24,
    session->shared_secret,
    HAP_X25519_KEY_SIZE,
    (uint8_t*)"Pair-Verify-Encrypt-Info",
    24,
    session_key,
    32);
  ...
  crypto_aead_chacha20poly1305_ietf_encrypt(encrypted, &encrypted_len, sub_tlv, tlv8_encoder_size(&sub_enc), NULL, 0, NULL, nonce, session_key);
...
hap_pair_verify_m3(hap_session_t* session, const uint8_t* input, size_t input_len, uint8_t* output, size_t output_capacity, size_t* output_len) {
  ...
  if (
    crypto_aead_chacha20poly1305_ietf_decrypt(decrypted, &decrypted_len, NULL, encrypted, encrypted_len, NULL, 0, nonce, session->encrypt_key) != 0) {
  ...
  hap_hkdf_sha512(
    (uint8_t*)"Control-Salt",
    12,
    session->shared_secret,
    HAP_X25519_KEY_SIZE,
    (uint8_t*)"Control-Read-Encryption-Key",
    27,
    session->encrypt_key,
    32);
  ...
  hap_hkdf_sha512(
    ...
    session->shared_secret,
    HAP_X25519_KEY_SIZE,
    (uint8_t*)"Control-Write-Encryption-Key",
    28,
    session->decrypt_key,
    32);
...
hap_pair_verify_m1_raw(hap_session_t* session, const uint8_t* input, size_t input_len, uint8_t* output, size_t output_capacity, size_t* output_len) {
  ...
  crypto_sign_detached(signature, NULL, signed_data, sizeof(signed_data), session->device_secret_key);
  ...
  mbedtls_aes_crypt_ctr(&aes_ctx, sizeof(signature), &nc_off, nonce_counter, stream_block, signature, output + 32);
...
hap_pair_verify_m3_raw(hap_session_t* session, const uint8_t* input, size_t input_len, uint8_t* output, size_t output_capacity, size_t* output_len) {
  ...
  mbedtls_aes_crypt_ctr(&aes_ctx, sizeof(client_signature), &nc_off, nonce_counter, stream_block, encrypted_sig, client_signature);
  mbedtls_aes_free(&aes_ctx);
