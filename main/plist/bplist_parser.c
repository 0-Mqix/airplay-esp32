...
static uint64_t read_be_int(const uint8_t* data, size_t bytes) {
  uint64_t val = 0;
  for (size_t i = 0; i < bytes; i++) { val = (val << 8) | data[i]; }
  return val;
}

static bool bplist_parse_trailer(
  const uint8_t* plist,
  size_t         plist_len,
  uint8_t*       offset_size,
  uint8_t*       ref_size,
  ...
  *offset_table_offset = read_be_int(trailer + 24, 8);
  ...
  return (*offset_size > 0 && *offset_size <= 8 && *ref_size > 0 && *ref_size <= 8 && *offset_table_offset < plist_len);
}

static uint64_t bplist_get_offset(const uint8_t* plist, uint64_t offset_table_offset, uint8_t offset_size, uint64_t obj_idx) {
  const uint8_t* entry = plist + offset_table_offset + obj_idx * offset_size;
  return read_be_int(entry, offset_size);
}

static bool bplist_read_string(const uint8_t* plist, size_t plist_len, uint64_t offset, char* out, size_t out_capacity) {
...
static bool bplist_read_data(const uint8_t* plist, size_t plist_len, uint64_t offset, uint8_t* out, size_t out_capacity, size_t* out_len) {
  if (offset >= plist_len) { return false; }
...
static bool bplist_read_data_len(const uint8_t* plist, size_t plist_len, uint64_t offset, size_t* out_len) {
  if (offset >= plist_len) { return false; }
...
static bool bplist_read_string_len(const uint8_t* plist, size_t plist_len, uint64_t offset, size_t* out_len) {
  if (offset >= plist_len) { return false; }
...
static bool bplist_read_int(const uint8_t* plist, size_t plist_len, uint64_t offset, int64_t* out) {
  if (offset >= plist_len) { return false; }
...
static bool bplist_read_real(const uint8_t* plist, size_t plist_len, uint64_t offset, double* out) {
  if (offset >= plist_len) { return false; }
...
static bool bplist_parse_count(const uint8_t* plist, size_t plist_len, uint64_t offset, size_t* count, size_t* header_len) {
  if (offset >= plist_len) { return false; }
...
static bool bplist_find_data_in_dict(
  const uint8_t* plist,
  size_t         plist_len,
  ...
  size_t dict_size = 0;
  size_t header_len = 0;
  if (!bplist_parse_count(plist, plist_len, dict_offset, &dict_size, &header_len)) { return false; }
...
static bool bplist_find_data_recursive(
  const uint8_t* plist,
  size_t         plist_len,
  uint64_t       obj_idx,
  uint64_t       offset_table_offset,
  ...
  uint64_t offset = bplist_get_offset(plist, offset_table_offset, offset_size, obj_idx);
  if (offset >= plist_len) { return false; }
  ...
  if (type == BPLIST_DICT) {
    size_t dict_size = 0;
    size_t header_len = 0;
    if (!bplist_parse_count(plist, plist_len, offset, &dict_size, &header_len)) { return false; }
    ...
    size_t count = 0;
    size_t header_len = 0;
    if (!bplist_parse_count(plist, plist_len, offset, &count, &header_len)) { return false; }
...
bool bplist_find_data(const uint8_t* plist, size_t plist_len, const char* key, uint8_t* out_data, size_t out_capacity, size_t* out_len) {
  ...
  if (!bplist_parse_trailer(plist, plist_len, &offset_size, &ref_size, &num_objects, &top_object, &offset_table_offset)) { return false; }
  ...
  return bplist_find_data_in_dict(plist, plist_len, top_offset, offset_table_offset, offset_size, ref_size, key, out_data, out_capacity, out_len);
}
  ...
  for (size_t i = 0; i < dict_size; i++) {
    ...
    if (bplist_read_string(plist, plist_len, key_offset, found_key, sizeof(found_key))) {
      if (strcmp(found_key, key) == 0) {
        uint64_t val_idx = read_be_int(val_refs + i * ref_size, ref_size);
        uint64_t val_offset = bplist_get_offset(plist, offset_table_offset, offset_size, val_idx);
        if (bplist_read_real(plist, plist_len, val_offset, out_value)) { return true; }
        int64_t int_val = 0;
...
bool bplist_get_streams_count(const uint8_t* plist, size_t plist_len, size_t* count) {
  ...
  *count = 0;
  ...
  for (uint64_t i = 0; i < num_objects; i++) {
    uint64_t offset = bplist_get_offset(plist, offset_table_offset, offset_size, i);
    if (offset >= plist_len) { continue; }
    char key[16];
    if (bplist_read_string(plist, plist_len, offset, key, sizeof(key))) {
      if (strcmp(key, "streams") == 0) {
        streams_key_offset = offset;
        if (!bplist_read_string_len(plist, plist_len, offset, &streams_key_len)) { return false; }
        break;
      }
    }
  }

  if (streams_key_len == 0) { return false; }
  ...
  for (size_t i = 0; i < dict_size; i++) {
    ...
    if (key_offset == streams_key_offset) {
      ...
      if ((val_marker & 0xF0) != BPLIST_ARRAY) { return false; }

      size_t array_count = 0;
      size_t header_len = 0;
      if (!bplist_parse_count(plist, plist_len, val_offset, &array_count, &header_len)) { return false; }
  ...
  *type = -1;
  ...
  for (size_t i = 0; i < dict_size; i++) {
    ...
    if (key_offset == streams_key_offset) {
      ...
      if (index >= array_count) { return false; }

      size_t array_pos = val_offset + header_len;
      if (array_pos + array_count * ref_size > plist_len) { return false; }

      uint64_t stream_idx = read_be_int(plist + array_pos + index * ref_size, ref_size);
      uint64_t stream_offset = bplist_get_offset(plist, offset_table_offset, offset_size, stream_idx);

      uint8_t stream_marker = plist[stream_offset];
      if ((stream_marker & 0xF0) != BPLIST_DICT) { return false; }

      size_t stream_dict_size = 0;
      size_t stream_header_len = 0;
      if (!bplist_parse_count(plist, plist_len, stream_offset, &stream_dict_size, &stream_header_len)) { return false; }

      size_t stream_pos = stream_offset + stream_header_len;
      if (stream_pos + stream_dict_size * 2 * ref_size > plist_len) { return false; }

      const uint8_t* stream_key_refs = plist + stream_pos;
      const uint8_t* stream_val_refs = plist + stream_pos + stream_dict_size * ref_size;

      for (size_t j = 0; j < stream_dict_size; j++) {
        uint64_t stream_key_idx = read_be_int(stream_key_refs + j * ref_size, ref_size);
        uint64_t stream_key_offset = bplist_get_offset(plist, offset_table_offset, offset_size, stream_key_idx);

        char stream_key[32];
        if (!bplist_read_string(plist, plist_len, stream_key_offset, stream_key, sizeof(stream_key))) { continue; }

        uint64_t stream_val_idx = read_be_int(stream_val_refs + j * ref_size, ref_size);
        uint64_t stream_val_offset = bplist_get_offset(plist, offset_table_offset, offset_size, stream_val_idx);
  ...
  *out_count = 0;
  ...
  for (size_t i = 0; i < dict_size; i++) {
    ...
    if (key_offset == streams_key_offset) {
      ...
      for (size_t j = 0; j < stream_dict_size && *out_count < out_capacity; j++) {
        uint64_t stream_key_idx = read_be_int(stream_key_refs + j * ref_size, ref_size);
        uint64_t stream_key_offset = bplist_get_offset(plist, offset_table_offset, offset_size, stream_key_idx);
        ...
        if (stream_val_type == BPLIST_INT) {
          ...
          int64_t int_val = 0;
          if (bplist_read_int(plist, plist_len, stream_val_offset, &int_val)) { info->int_value = int_val; }
        } else if (stream_val_type == BPLIST_DATA) {
          info->value_type = BPLIST_VALUE_DATA;
          size_t len = 0;
          if (bplist_read_data_len(plist, plist_len, stream_val_offset, &len)) { info->value_len = len; }
        } else if (stream_val_type == BPLIST_STRING || stream_val_type == BPLIST_UNICODE) {
          info->value_type = BPLIST_VALUE_STRING;
          size_t len = 0;
          if (bplist_read_string_len(plist, plist_len, stream_val_offset, &len)) { info->value_len = len; }
...
bool bplist_find_stream_crypto(
  ...
  if (!bplist_get_streams_count(plist, plist_len, &stream_count)) { return false; }
  ...
  for (size_t i = 0; i < stream_count; i++) {
    ...
    if (ekey && local_ekey_len > 0 && ekey_len) {
      if (bplist_find_data(plist, plist_len, "ekey", temp_buf, sizeof(temp_buf), &temp_len)) {
    ...
    if (eiv && local_eiv_len > 0 && eiv_len) {
      if (bplist_find_data(plist, plist_len, "eiv", temp_buf, sizeof(temp_buf), &temp_len)) {
    ...
    if (shk && local_shk_len > 0 && shk_len) {
      if (bplist_find_data(plist, plist_len, "shk", temp_buf, sizeof(temp_buf), &temp_len)) {
