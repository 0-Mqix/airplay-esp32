...
void plist_dict_begin(plist_t* p) { plist_append(p, "<dict>\n"); }
...
void plist_dict_string(plist_t* p, const char* key, const char* value) {
  ...
  if (len > 0 && (size_t)len < remaining) { p->size += (size_t)len; }
}
...
void plist_dict_data(plist_t* p, const char* key, const uint8_t* data, size_t len) {
  ...
  if (remaining < strlen(key) + b64_len + 50) { return; }

  int written = snprintf(p->buffer + p->size, remaining, "<key>%s</key>\n<data>", key);
  if (written > 0) { p->size += (size_t)written; }

  int encoded = base64_encode(data, len, p->buffer + p->size, p->capacity - p->size);
  if (encoded < 0) { return; }
...
void plist_dict_data_hex(plist_t* p, const char* key, const uint8_t* data, size_t len) { plist_dict_data(p, key, data, len); }

void plist_dict_end(plist_t* p) { plist_append(p, "</dict>\n"); }
...
void plist_array_begin(plist_t* p) { plist_append(p, "<array>\n"); }

void plist_array_end(plist_t* p) { plist_append(p, "</array>\n"); }

void plist_array_int(plist_t* p, int64_t value) {
  size_t remaining = p->capacity - p->size;
  int    len = snprintf(p->buffer + p->size, remaining, "<integer>%" PRId64 "</integer>\n", value);
  if (len > 0 && (size_t)len < remaining) { p->size += (size_t)len; }
}
