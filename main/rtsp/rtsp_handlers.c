...
static void configure_codec(audio_format_t* fmt, const char* name, int64_t sr, int64_t spf) {
...
static const rtsp_codec_t codec_registry[] = {{"ALAC", 2}, {"AAC", 4}, {"AAC-ELD", 8}, {"OPUS", 64}, {NULL, 0}};

bool rtsp_codec_configure(int64_t type_id, audio_format_t* fmt, int64_t sample_rate, int64_t samples_per_frame) {
  for (const rtsp_codec_t* codec = codec_registry; codec->name; codec++) {
    if (codec->type_id == type_id) {
      ...
      ESP_LOGI(
        TAG,
        "Configured codec: %s (ct=%lld, sr=%lld, spf=%lld)",
        codec->name,
        (long long)type_id,
  ...
  ESP_LOGW(TAG, "Unknown codec type %lld, defaulting to ALAC", (long long)type_id);
...
void rtsp_get_device_id(char* device_id, size_t len) {
  ...
  snprintf(device_id, len, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
...
static void event_port_task(void* pvParameters) {
  ...
  while (!event_task_should_stop && listen_socket >= 0) {
    ...
    if (ret < 0) {
      if (errno != EINTR && !event_task_should_stop) { ESP_LOGE(TAG, "Event port select error: %d", errno); }
      break;
    }
    ...
    if (FD_ISSET(listen_socket, &read_fds)) {
      ...
      if (client < 0) {
        if (!event_task_should_stop) { ESP_LOGE(TAG, "Event port accept error: %d", errno); }
        break; // Don't continue - socket is likely invalid
      }

      if (event_client_socket >= 0) { close(event_client_socket); }
...
void rtsp_start_event_port_task(int listen_socket) {
  if (event_task_handle != NULL) { return; }
  event_task_should_stop = false;
  event_listen_socket = -1;
  xTaskCreatePinnedToCore(
    event_port_task,
    "event_port",
    TASK_EVENT_PORT_STACK,
    (void*)(intptr_t)listen_socket,
    TASK_EVENT_PORT_PRIORITY,
    &event_task_handle,
    TASK_EVENT_PORT_CORE);
}
...
static void handle_options(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_get(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_post(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_announce(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_setup(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_record(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_set_parameter(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_get_parameter(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_pause(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_flush(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_teardown(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
static void handle_setrateanchortime(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len);
...
int rtsp_dispatch(int socket, rtsp_conn_t* conn, const uint8_t* raw_request, size_t raw_len) {
  rtsp_request_t req;
...
static void handle_get(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len) {
  ...
  if (strcmp(req->path, "/info") == 0) {
    ...
    rtsp_send_http_response(socket, conn, 200, "OK", "text/x-apple-plist+xml", body, body_len);
  } else {
    ESP_LOGW(TAG, "Unknown GET path: %s", req->path);
    rtsp_send_http_response(socket, conn, 404, "Not Found", "text/plain", "Not Found", 9);
  }
}
  ...
  if (strstr(req->path, "/pair-setup")) {
    ...
    if (body && body_len > 0) {
      ...
      if (state && state_len == 1) {
        switch (state[0]) {
          case 1: err = hap_pair_setup_m1(conn->hap_session, body, body_len, response, 2048, &response_len); break;
          case 3: err = hap_pair_setup_m3(conn->hap_session, body, body_len, response, 2048, &response_len); break;
          case 5: err = hap_pair_setup_m5(conn->hap_session, body, body_len, response, 2048, &response_len); break;
        }
      }
    }
    ...
    if (err == ESP_OK && response_len > 0) {
      ...
      if (conn->hap_session && conn->hap_session->pair_setup_state == 4 && conn->hap_session->session_established) { conn->encrypted_mode = true; }
    } else {
    ...
    if (body && body_len > 0) {
      ...
      if (state && state_len == 1) {
        if (state[0] == 0x01) {
          err = hap_pair_verify_m1(conn->hap_session, body, body_len, response, 1024, &response_len);
        } else if (state[0] == 0x03) {
          err = hap_pair_verify_m3(conn->hap_session, body, body_len, response, 1024, &response_len);
        }
      } else {
        // Raw format - used for audio encryption keys, not RTSP encryption
        if (conn->hap_session->pair_verify_state == 0) {
          err = hap_pair_verify_m1_raw(conn->hap_session, body, body_len, response, 1024, &response_len);
        } else if (conn->hap_session->pair_verify_state == PAIR_VERIFY_STATE_M2) {
          err = hap_pair_verify_m3_raw(conn->hap_session, body, body_len, response, 1024, &response_len);
          if (err == ESP_OK) { ESP_LOGI(TAG, "Raw pair-verify complete (RTSP unencrypted)"); }
        }
      }
    }
    ...
    if (err == ESP_OK && response_len > 0) {
      ...
      if (conn->hap_session && conn->hap_session->pair_verify_state == PAIR_VERIFY_STATE_M4) {
    ...
    if (body && body_len >= 16) {
      if (rtsp_fairplay_handle(body, body_len, &fp_response, &fp_response_len) == 0) {
        rtsp_send_response(
          socket,
          conn,
          200,
    ...
    if (body && body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
      int64_t cmd_type = 0;
      if (bplist_find_int(body, body_len, "type", &cmd_type)) { ESP_LOGI(TAG, "/command type=%lld", (long long)cmd_type); }
    }
    ...
    if (body && body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
      int64_t value;
      if (bplist_find_int(body, body_len, "networkTimeSecs", &value)) { ESP_LOGI(TAG, "/feedback has networkTimeSecs=%lld", (long long)value); }
    }
    rtsp_send_ok(socket, conn, req->cseq);
...
static void parse_sdp(rtsp_conn_t* conn, const char* sdp, size_t len) {
    ...
    if (slash) {
      *slash = '\0';
      ...
      if (sscanf(slash + 1, "%d/%d", &sr, &ch) >= 1) {
        if (sr > 0) { format.sample_rate = sr; }
        if (ch > 0) { format.channels = ch; }
      }
    }
  }
  ...
  if (fmtp) {
    ...
    int          matched = sscanf(
      fmtp,
      "a=fmtp:%*d %u %u %u %u %u %u %u %u %u %u %u",
      &frame_len,
      &compat,
      &bit_depth,
      &pb,
  ...
  if (
    (strstr(format.codec, "AAC") || strstr(format.codec, "aac") || strstr(format.codec, "mpeg4-generic") || strstr(format.codec, "MPEG4-GENERIC")) &&
  ...
  if (encrypt.type != AUDIO_ENCRYPT_NONE) { audio_receiver_set_encryption(&encrypt); }
}
...
static void handle_announce(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len) {
  ...
  if (req->body && req->body_len > 0) { parse_sdp(conn, (const char*)req->body, req->body_len); }
...
static void handle_setup(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len) {
  ...
  if (body && body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
    if (bplist_get_streams_count(body, body_len, &stream_count)) { request_has_streams = true; }
  }

  ESP_LOGI(
    TAG,
    "SETUP: has_streams=%d, stream_count=%zu, is_bplist=%d, content_type='%s'",
    request_has_streams,
    stream_count,
    is_bplist,
    req->content_type);
  ...
  if (body && body_len > 0) {
    ...
    if (shk_len >= 16) {
      ...
      if (eiv_len >= 16) { memcpy(audio_encrypt.iv, eiv, 16); }
      audio_receiver_set_encryption(&audio_encrypt);
      encryption_set = true;
    } else if (ekey_len > 16 && conn->hap_session && conn->hap_session->session_established) {
      uint8_t            nonce[12] = {0};
      ...
      if (
        crypto_aead_chacha20poly1305_ietf_decrypt(
          ...
          0,
          nonce,
          conn->hap_session->shared_secret) == 0 &&
        decrypted_len >= 16) {
        audio_encrypt.type = AUDIO_ENCRYPT_CHACHA20_POLY1305;
    ...
    if (!encryption_set && conn->hap_session && conn->hap_session->session_established) {
      audio_encrypt.type = AUDIO_ENCRYPT_CHACHA20_POLY1305;
      if (hap_derive_audio_key(conn->hap_session, audio_encrypt.key, sizeof(audio_encrypt.key)) == ESP_OK) {
  ...
  if (stream_type == 0) { stream_type = 96; }
  ...
  if (is_bplist) {
    ...
    size_t  plist_len =
      bplist_build_stream_setup(plist_body, sizeof(plist_body), stream_type, response_data_port, conn->control_port, AP2_AUDIO_BUFFER_SIZE);
    if (plist_len == 0) {
      rtsp_send_response(socket, conn, 500, "Internal Error", req->cseq, NULL, NULL, 0);
      return;
    }
    ESP_LOGI(TAG, "SETUP response: type=%lld dataPort=%u controlPort=%u", (long long)stream_type, response_data_port, conn->control_port);
    ...
    rtsp_parse_transport((const char*)raw, &conn->client_control_port, &conn->client_timing_port);
    ESP_LOGI(TAG, "Client ports: control=%u timing=%u", conn->client_control_port, conn->client_timing_port);
    ...
    snprintf(
      ...
      conn->control_port,
      conn->timing_port);
    rtsp_send_response(socket, conn, 200, "OK", req->cseq, transport_response, NULL, 0);
  }
...
static void handle_set_parameter(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len) {
  ...
  if (strstr(req->content_type, "application/x-apple-binary-plist")) {
    if (body && body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
      ...
      double rate;
      if (bplist_find_real(body, body_len, "rate", &rate)) { ESP_LOGI(TAG, "SET_PARAMETER has rate=%.2f", rate); }
    }
  }
...
static void handle_teardown(int socket, rtsp_conn_t* conn, const rtsp_request_t* req, const uint8_t* raw, size_t raw_len) {
  ...
  if (body && body_len >= 8 && memcmp(body, "bplist00", 8) == 0) {
    if (bplist_get_streams_count(body, body_len, &stream_count)) { has_streams = true; }
  }
  ...
  if (!has_streams) { conn->timing_port = 0; }
