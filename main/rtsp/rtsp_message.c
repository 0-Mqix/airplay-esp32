...
void rtsp_parse_transport(const char* request, uint16_t* control_port, uint16_t* timing_port) {
  ...
  if (!line_end) { line_end = transport + strlen(transport); }

  // Parse control_port
  const char* cp = strstr(transport, "control_port=");
  if (cp && cp < line_end && control_port) { *control_port = (uint16_t)atoi(cp + 13); }

  // Parse timing_port
  const char* tp = strstr(transport, "timing_port=");
  if (tp && tp < line_end && timing_port) { *timing_port = (uint16_t)atoi(tp + 12); }
}

int rtsp_request_parse(const uint8_t* data, size_t len, rtsp_request_t* req) {
  if (!data || !req || len == 0) { return -1; }
  ...
  if (sscanf((const char*)data, "%31s %255s", req->method, req->path) < 1) { return -1; }
  ...
  if (ct) { sscanf(ct, "Content-Type: %63s", req->content_type); }
...
int rtsp_send_response(
  ...
  if (body && body_len > 0) { memcpy(response + header_len, body, body_len); }
  ...
  if (conn && conn->encrypted_mode) {
    ...
    if (result < 0) { ESP_LOGE(TAG, "Failed to send RTSP response"); }
  }
...
int rtsp_send_ok(int socket, rtsp_conn_t* conn, int cseq) { return rtsp_send_response(socket, conn, 200, "OK", cseq, NULL, NULL, 0); }
...
int rtsp_send_http_response(
  ...
  if (conn && conn->encrypted_mode) {
    ...
    result = (send_all(socket, response, total_len) < 0) ? -1 : 0;
    if (result < 0) { ESP_LOGE(TAG, "Failed to send HTTP response"); }
  }
